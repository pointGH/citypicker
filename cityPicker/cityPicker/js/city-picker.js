...
(function (factory) {
    ...
    function CityPicker(element,dataConfig,options) {
    ...
    function orderData(dataConfig){
        var parentCodeName = dataConfig.parentCodeName;
        var codeName = dataConfig.codeName;
        var rootCode = dataConfig.rootCode;
        var showName = dataConfig.showName;
        var data = dataConfig.data;
        var resultData = [];
        var tempData = [];
        data.forEach(function(value){
            if(value[parentCodeName]==rootCode)
            {
        ...
        resultData.forEach(function(value){
            ...
            data.forEach(function(subvalue){
                if(subvalue[parentCodeName]==value[codeName])
                {
                    value.child.push(subvalue);
                }
                else
                {
                    tempData.push(value);
        ...
        resultData.forEach(function(value){
            value.child.forEach(function(subvalue){
                subvalue.child = [];
                data.forEach(function(svalue){
                    if(subvalue[codeName]==svalue[parentCodeName])
                    {
                        subvalue.child.push(svalue);
                    }
                })               
            })
        });
    ...
    CityPicker.prototype = {
        ...
        output: function (type) {
            ...
            code = (
                type === PROVINCE ? dataConfig.rootCode :
                    type === CITY ? this.$province && this.$province.find('.active').data('code') :
                        type === DISTRICT ? this.$city && this.$city.find('.active').data('code') : code
            );
            dataConfig.data.forEach(function(record){
                if(record[dataConfig.parentCodeName] == code)
                    districts.push(record);
            })
            districts.forEach(function (tvalue) {
                var code = tvalue[dataConfig.codeName];
                var address = tvalue[dataConfig.showName];
                if (address === value) {
                    matched = {
                        code: code,
                        address: address
        ...
        getProvinceList: function (data) {//html
            var list = [],
                $this = this,
